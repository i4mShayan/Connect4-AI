        ...
        for event in pygame.event.get():
            ...
            if globals.current_state == IN_MENU:
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if check_button_click(start_game_x, start_game_y, BUTTON_WIDTH, BUTTON_HEIGHT, mouse_pos):
                        globals.current_state = IN_GAME
                        globals.PLAYER_PIECE = RED_PIECE
                        globals.AI_PIECE = YELLOW_PIECE

                    elif check_button_click(start_agent_game_x, start_agent_game_y, BUTTON_WIDTH, BUTTON_HEIGHT, mouse_pos):
                        globals.current_state = IN_GAME
                        globals.PLAYER_PIECE = YELLOW_PIECE
                        globals.AI_PIECE = RED_PIECE

                    elif check_button_click(creator_x, creator_y, BUTTON_WIDTH, BUTTON_HEIGHT, mouse_pos):
                        globals.current_state = IN_CREATOR

                    elif check_button_click(button1_x, settings_y, BUTTON_WIDTH // 3, BUTTON_HEIGHT, mouse_pos):
                        globals.settings_button_colors[0] = [GREEN, GREY, GREY]
                        globals.LEVEL = EASY

                    elif check_button_click(button2_x, settings_y, BUTTON_WIDTH // 3, BUTTON_HEIGHT, mouse_pos):
                        globals.settings_button_colors[0] = [GREY, GREEN, GREY]
                        globals.LEVEL = MEDIUM

                    elif check_button_click(button3_x, settings_y, BUTTON_WIDTH // 3, BUTTON_HEIGHT, mouse_pos):
                        globals.settings_button_colors[0] = [GREY, GREY, GREEN]
                        globals.LEVEL = HARD

                    elif check_button_click(button4_x, settings2_y, BUTTON_WIDTH // 3, BUTTON_HEIGHT, mouse_pos):
                        globals.settings_button_colors[1] = [GREEN, GREY, GREY]
                        globals.ALGORITHM = NORMAL_MINIMAX

                    elif check_button_click(button5_x, settings2_y, BUTTON_WIDTH // 3, BUTTON_HEIGHT, mouse_pos):
                        globals.settings_button_colors[1] = [GREY, GREEN, GREY]
                        globals.ALGORITHM = ALPHABETA_MINIMAX

                    elif check_button_click(button6_x, settings2_y, BUTTON_WIDTH // 3, BUTTON_HEIGHT, mouse_pos):
                        globals.settings_button_colors[1] = [GREY, GREY, GREEN]
                        globals.ALGORITHM = Q_LEARNING
                ...
                if globals.turn == globals.PLAYER_PIECE:
                    if event.type == pygame.MOUSEBUTTONDOWN:
                        ...
                        if is_valid_move(globals.board, col):
